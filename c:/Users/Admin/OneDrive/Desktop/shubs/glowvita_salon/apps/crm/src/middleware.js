...
export async function middleware(request) {
  ...
  // Check if the user is a staff member and has permission for the requested path.
  if (role === 'staff') {
    const requiredPermission = navItems.find(item => pathname.startsWith(item.href))?.permission;
    if (requiredPermission && !permissions.includes(requiredPermission)) {
      return NextResponse.redirect(new URL('/not-found', request.url));
    }
  }

  // General path validation for all roles
