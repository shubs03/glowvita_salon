...
export default function CanvasTemplateEditor({ 
  ...
  const initCanvas = useCallback(() => {
    ...
    canvas.on('selection:created', (e: TEvent<MouseEvent>) => {
      const selected = (e as any).selected;
      if (selected && selected.length > 0) {
        setSelectedObject(selected[0]);
        if (selected[0] && selected[0].type === 'textbox') {
          updateTextControls(selected[0]);
        }
      }
    });
    
    canvas.on('selection:updated', (e: TEvent<MouseEvent>) => {
      const selected = (e as any).selected;
      if (selected && selected.length > 0) {
        setSelectedObject(selected[0]);
        if (selected[0] && selected[0].type === 'textbox') {
