...
export function SalonsSection() {
  ...
  const salons = (vendorsData || []).map((vendor: any) => ({
    name: vendor.businessName,
    rating: vendor.rating || 4.7, // Default rating if not available
    location: `${vendor.city}, ${vendor.state}`,
    image: vendor.profileImage || 'https://placehold.co/600x400.png',
    hint: `salon in ${vendor.city}`,
    topRated: (vendor.rating || 0) >= 4.8,
    services: vendor.services?.slice(0, 2).map((s: any) => s.name) || ['Hair Styling', 'Facials'], // Default services
    price: vendor.priceRange || '₹2,000+', // Default price range
  }));
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className={cn(
          ...
          {(isLoading ? [...Array(4)] : salons).map((salon, index) => (
            <div
              key={salon?._id || index}
            ...
            >
              {isLoading ? (
                <div className="aspect-[4/5] rounded-lg bg-muted animate-pulse"></div>
              ) : (
                <SalonCard {...salon} />
              )}
            </div>
