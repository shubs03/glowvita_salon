...
export const GET = authMiddlewareAdmin(async (req) => {
  try {
    ...
    if (uniqueVendors.length > 0) {
      ...
      const DoctorModel = await import('@repo/lib/models/Vendor/Docters.model').then(module => module.default);

      const [vendors, doctors] = await Promise.all([
        VendorModel.find({ businessName: { $in: uniqueVendors } }, 'businessName type'),
        DoctorModel.find({ name: { $in: uniqueVendors } }, 'name')
      ]);
      ...
      doctors.forEach(doctor => {
        vendorTypesMap.set(doctor.name, 'Doctor');
      });
    }
