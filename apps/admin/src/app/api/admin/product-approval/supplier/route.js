  ...
  try {
    ...
    (async () => {
      try {
        ...
        await NotificationService.sendDocumentAlert(updatedProduct.vendorId._id.toString(), 'supplier', `Product "${updatedProduct.productName}"`, status, rejectionReason, 'Product');
      } catch (err) {
