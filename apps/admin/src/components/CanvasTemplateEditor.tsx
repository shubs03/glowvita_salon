...
async function urlToDataUrl(src: string): Promise<string> {
  ...
  try {
    const res = await fetch(absUrl, { mode: 'cors', signal: controller.signal });
...
const CanvasTemplateEditor = forwardRef<CanvasTemplateEditorRef, CanvasTemplateEditorProps>(({
  initialImage, initialJsonData, onSaveTemplate, width = 680
}, ref) => {
  const [fab, setFab] = useState<any>(null);
  const [isReady, setIsReady] = useState(false);
  const [sel, setSel] = useState<any>(null);
  const [cssScale, setCssScale] = useState(1);
  const [canvasDims, setCanvasDims] = useState({ w: width, h: 500 });
  ...
  useEffect(() => {
    ...
    const addBg = (src: string, done: () => void) => {
      ...
      const loadAsDataUrl = (url: string): Promise<string> => {
