...
// GET - Fetch sales by customer report
export const GET = authMiddlewareCrm(async (req) => {
  try {
    ...
    const salesByCustomerArray = Object.values(salesByCustomer).map(customerData => ({
      ...
      serviceSold: customerData.serviceSold,
      grossSale: parseFloat(customerData.grossSale.toFixed(2)),
      discounts: parseFloat(customerData.discounts.toFixed(2)),
    ...
    const previewData = [];
    Object.values(salesByCustomer).forEach((customerData, index) => {
      ...
      if (latestAppointment) {
        previewData.push({
          ...
          amount: parseFloat(latestAppointment.amount.toFixed(2)),
