...
// GET - Fetch pending orders report for supplier
export const GET = authMiddlewareCrm(async (request) => {
  try {
    const supplierId = request.user.userId;
    
    // Get all pending orders where this supplier is the seller
    ...
    const totalPendingRevenue = orders.reduce((sum, order) => sum + (order.totalAmount || 0), 0);
    ...
    return NextResponse.json({
      ...
      message: "Supplier pending orders report fetched successfully"
    }, { status: 200 });

  } catch (error) {
    console.error("Error fetching supplier pending orders report:", error);
    return NextResponse.json({
      success: false,
      message: "Failed to fetch supplier pending orders report",
      error: error.message
    }, { status: 500 });
  }
}, ['supplier']);