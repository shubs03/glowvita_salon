...
const initialVendorData = {
  ...
  description: "Your one-stop destination for beauty and wellness. We offer a wide range of services from haircuts to spa treatments.",
  ...
  documents: [
    { id: '1', name: 'aadhar_card.pdf', type: 'aadhar' as const, status: 'approved' as const },
    { id: '2', name: 'pan_card.pdf', type: 'pan' as const, status: 'approved' as const },
    { id: '3', name: 'gst_cert.pdf', type: 'gst' as const, status: 'pending' as const },
  ],
  openingHours: [
  ...
  <Card>
    ...
    <CardContent className="space-y-6">
      <div className="space-y-2">
        ...
        <Input id="businessName" value={vendor.businessName} onChange={(e) => setVendor({ ...vendor, businessName: e.target.value })} />
      </div>
      <div className="space-y-2">
        <Label htmlFor="description">Description</Label>
        <Textarea id="description" value={vendor.description} onChange={(e) => setVendor({ ...vendor, description: e.target.value })} />
      </div>
       <div className="space-y-2">
          <Label>Salon Category</Label>
          <Select value={vendor.category} onValueChange={(value) => setVendor({ ...vendor, category: value })}>
                  ...
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => {
                       const newSubCats = checked 
                         ...
                         : vendor.subCategories.filter((id: string) => id !== subCat.id);
                       setVendor({...vendor, subCategories: newSubCats});
                    }}
                  />
                  <Label htmlFor={subCat.id} className="text-sm font-medium leading-none">
                    {subCat.label}
                  </Label>
                </div>
              ))}
    ...
    <CardContent className="space-y-4">
        <div className="flex justify-between items-center p-4 border rounded-lg">
            <div>
                ...
                <p className="text-sm text-muted-foreground">Status: <span className={subscription.status === 'Active' ? 'text-green-600' : 'text-red-600'}>{subscription.status}</span></p>
            </div>
  ...
  <Card>
    ...
    <CardContent>
        <div className="mb-6 p-6 border-2 border-dashed rounded-lg text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Drag & drop images here or</p>
            <Button variant="link" asChild><label htmlFor="gallery-upload" className="cursor-pointer">browse to upload</label></Button>
            <Input id="gallery-upload" type="file" className="hidden" multiple accept="image/*" />
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {gallery.map((src, index) => (
                <div key={index} className="relative group aspect-video">
                    <Image src={src} alt={`Salon image ${index + 1}`} layout="fill" className="object-cover rounded-lg" />
  ...
  <Card>
    ...
    <CardContent>
        <div className="mb-6 p-6 border-2 border-dashed rounded-lg text-center">
            ...
            <Button variant="link" asChild><label htmlFor="doc-upload" className="cursor-pointer">browse to upload</label></Button>
            <Input id="doc-upload" type="file" className="hidden" accept=".pdf,.jpg,.png" />
        </div>
        <ul className="space-y-3">
            {documents.map(doc => (
                <li key={doc.id} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <div className="flex items-center gap-3">
                        <Badge variant={doc.status === 'approved' ? 'default' : 'secondary'} className={doc.status === 'approved' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}>
...
const OpeningHoursTab = ({ hours, setHours }: { hours: OpeningHour[], setHours: any }) => (
    <Card>
        <CardHeader>
            ...
            {hours.map((hour, index) => (
                <div key={hour.day} className="grid grid-cols-4 items-center gap-4">
                    <div className="col-span-1">
                        ...
                        />
                        <Label htmlFor={hour.day} className="ml-2 font-medium">{hour.day}</Label>
                    </div>
                    <div className="col-span-1">
                       <Input 
                         type="time" 
                    ...
                    <div className="col-span-1 text-right">
                      {hour.isOpen ? <span className="text-green-600">Open</span> : <span className="text-red-600">Closed</span>}
                    </div>
                </div>
            ))}
        </CardContent>
...
export default function SalonProfilePage() {
    ...
    const [openingHours, setOpeningHours] = useState(initialVendorData.openingHours);
        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
                ...
                <div className="bg-muted/30 p-6">
                    <div className="flex flex-col md:flex-row items-start gap-6">
                        <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-background shadow-lg flex-shrink-0">
                            <Image src="https://placehold.co/200x200.png" alt="Salon Logo" layout="fill" className="object-cover" />
                        </div>
                        <div className="flex-grow">
                            <CardTitle className="text-2xl md:text-3xl font-bold">{vendor.businessName}</CardTitle>
                            <CardDescription className="text-base flex items-center gap-2 mt-1">
                                <MapPin className="h-4 w-4" /> {vendor.address}
                            </CardDescription>
                            <div className="text-sm text-muted-foreground mt-2">
                                Salon ID: <span className="font-mono bg-secondary px-1.5 py-0.5 rounded">{vendor.salonId}</span>
                            </div>
            ...
            <Tabs defaultValue="profile" className="w-full">
                ...
                <TabsContent value="profile" className="mt-4">
                    <ProfileTab vendor={vendor} setVendor={setVendor} categories={[]} subCategories={subCategories} />
                </TabsContent>
