...
import { Plus, Eye, Trash2, Send, MessageSquare, Users, Search, X } from 'lucide-react';
import { RadioGroup, RadioGroupItem } from '@repo/ui/radio-group';
import { Checkbox } from '@repo/ui/checkbox';
import { Badge } from '@repo/ui/badge';
import { Skeleton } from '@repo/ui/skeleton';
import toast from 'react-hot-toast';
import { useGetVendorNotificationsQuery, useCreateVendorNotificationMutation, useDeleteVendorNotificationMutation } from '@repo/store/api';
...
interface Notification {
  ...
  channels: string[];
  content: string;
  targetType: 'all_online_clients' | 'all_offline_clients' | 'all_staffs' | 'specific_clients';
  targets?: Client[];
  date: string;
...
export default function PushNotificationsPage() {
  ...
  const [targetType, setTargetType] = useState<'all_online_clients' | 'all_offline_clients' | 'all_staffs' | 'specific_clients'>('all_online_clients');
  ...
  const vendorId = useSelector((state: any) => state.auth?.vendorId) || 'vendor_id_from_auth';
  ...
  const [createNotification, { isLoading: isCreating }] = useCreateVendorNotificationMutation();
  const [deleteNotification] = useDeleteVendorNotificationMutation();

  const notifications = data?.notifications || [];
  const totalPages = data?.pagination?.totalPages || 0;
  const stats = data?.stats || { total: 0, pushSent: 0, smsSent: 0, mostTargeted: 'All Online Clients' };

  const filteredTargets = useMemo(() => 
    mockClients.filter(t => 
  ...
  const handleOpenModal = (type: 'add' | 'view', notification: Notification | null = null) => {
    setModalType(type);
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? <Skeleton className="h-8 w-12" /> : <div className="text-2xl font-bold">{stats.pushSent}</div>}
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? <Skeleton className="h-8 w-12" /> : <div className="text-2xl font-bold">{stats.smsSent}</div>}
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? <Skeleton className="h-8 w-24" /> : <div className="text-2xl font-bold">{stats.mostTargeted}</div>}
                  ...
                  <TableRow>
                    <TableCell colSpan={6}>
                      <div className="text-center py-4">
                        <p className="text-red-500">Error in loading Data</p>
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Special Weekend Offer" required disabled={modalType === 'view'} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="content">Content</Label>
              <Textarea id="content" value={content} onChange={(e) => setContent(e.target.value)} placeholder="Enter notification content here..." required disabled={modalType === 'view'} />
            </div>
            <div className="space-y-2">
              <Label>Target Audience</Label>
              <RadioGroup value={targetType} onValueChange={setTargetType} className="flex gap-4" disabled={modalType === 'view'}>
                <div className="flex items-center space-x-2"><RadioGroupItem value="all_online_clients" id="all_online_clients" /><Label htmlFor="all_online_clients">All Online Clients</Label></div>
                <div className="flex items-center space-x-2"><RadioGroupItem value="all_offline_clients" id="all_offline_clients" /><Label htmlFor="all_offline_clients">All Offline Clients</Label></div>
                ...
                <div className="max-h-40 overflow-y-auto border rounded-md p-2">
                  {filteredTargets.map((target) => (
                    <div key={target.id} className="flex items-center justify-between py-1">
                      ...
                      {modalType !== 'view' && (
                        <Button variant="ghost" size="sm" onClick={() => handleSelectTarget(target)}>
                          <Plus className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
                  ))}
                  {filteredTargets.length === 0 && <p className="text-center text-muted-foreground">No results found.</p>}
                </div>
                <div className="flex flex-wrap gap-2 mt-2">
                  {selectedTargets.map((target) => (
                    <Badge key={target.id} variant="secondary" className="flex items-center">
                      {target.name}
                      {modalType !== 'view' && (
                        <Button variant="ghost" size="icon" className="ml-2 h-4 w-4" onClick={() => handleRemoveTarget(target.id)}>
                          <X className="h-3 w-3" />
                        </Button>
                      )}
                    </Badge>
          ...
          <DialogFooter>
            ...
            {modalType !== 'view' && <Button onClick={handleSubmit} disabled={isCreating}>Send Notification</Button>}
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
