...
interface InvoiceUIProps {
  ...
  onEmailClick: () => void;
  onRebookClick: () => void;
  // Optional extras used by ServicesTab / ProductsTab
  onPrintClick?: () => void;
...
export function InvoiceUI({
  ...
  return (
    <div
      id="invoice-content"
      className="max-w-4xl mx-auto p-4 sm:p-6 bg-white font-sans print:p-0 print:max-w-none print:w-full rounded-lg print:rounded-none"
      style={{ minWidth: "auto" }}
    >
      {/* GlowVita Branding */}
      <div className="bg-slate-50 text-slate-900 py-3 px-4 rounded-t-lg -mx-4 sm:-mx-6 -mt-4 sm:-mt-6 mb-4 border-b border-slate-200 print:mx-0 print:mt-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start mb-4 print:mb-3 border-b-2 border-black pb-4 gap-3 sm:gap-0">
        <div className="w-full sm:w-auto">
          <h1 className="text-lg sm:text-xl font-bold text-black print:text-lg">
            {vendorName}
          </h1>
          <div
            className="text-black text-xs sm:text-sm mt-1 print:text-xs"
            dangerouslySetInnerHTML={{ __html: formatAddress(vendorAddress) }}
          />
          <p className="text-black text-xs sm:text-sm mt-1 print:text-xs">
            Phone: {vendorPhone}
          </p>
        </div>
        <div className="text-center w-full sm:w-auto">
          <h2 className="text-xl sm:text-2xl font-bold text-black print:text-xl">
            INVOICE
          </h2>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 print:mb-3 gap-1 sm:gap-0">
        <div>
          <p className="text-black text-xs sm:text-sm print:text-xs">
      ...
      <div className="mb-4 print:mb-3">
        <p className="text-black text-xs sm:text-sm print:text-xs">
          ...
          {invoiceData.client?.fullName || "N/A"}
        </p>
        ...
        <table className="w-full border-collapse border border-black min-w-[600px] sm:min-w-0">
          <thead>
            <tr className="bg-gray-200">
              <th className="border border-black p-1 sm:p-2 text-left text-xs sm:text-sm font-bold text-black print:text-xs print:p-1">
                ITEM DESCRIPTION
              </th>
              <th className="border border-black p-1 sm:p-2 text-right text-xs sm:text-sm font-bold text-black print:text-xs print:p-1">
                ₹ PRICE
              </th>
              <th className="border border-black p-1 sm:p-2 text-right text-xs sm:text-sm font-bold text-black print:text-xs print:p-1">
                QTY
          ...
          <tbody>
            ...
            <tr className="border border-black">
              <td
                className="border border-black p-1 sm:p-2 text-right font-semibold text-black text-xs sm:text-sm print:text-xs print:p-1"
                colSpan={4}
              >
                Subtotal:
              </td>
              <td className="border border-black p-1 sm:p-2 text-right text-xs sm:text-sm font-semibold text-black print:text-xs print:p-1">
                ₹
                {(
                  invoiceData.originalSubtotal ?? invoiceData.subtotal
                ).toFixed(2)}
              </td>
            </tr>
            ...
            <tr className="border border-black bg-gray-200">
              <td
                className="border border-black p-1 sm:p-2 text-right font-bold text-black text-xs sm:text-sm print:text-xs print:p-1"
                colSpan={4}
              >
                Total:
              </td>
              <td className="border border-black p-1 sm:p-2 text-right font-bold text-black text-xs sm:text-sm print:text-xs print:p-1">
      ...
      <div className="border-t-2 border-black pt-4 print:pt-3 mt-auto">
        ...
        <p className="text-center text-xs sm:text-sm font-semibold text-gray-600 print:text-xs uppercase tracking-wider px-2">
          NOTE: This is computer generated receipt and does not require physical
          signature.
        </p>

        {/* GlowVita Footer Branding */}
        <div className="mt-6 pt-4 border-t border-gray-300 print:hidden">
          <div className="flex flex-col items-center justify-center gap-2 text-gray-900">
