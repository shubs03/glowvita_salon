...
const AllSalons: React.FC<AllSalonsProps> = ({ maxSalons = 8 }) => {
  ...
  const salons = React.useMemo(() => {
    if (!landingData?.data?.all) return [];
    return landingData.data.all.map(transformVendor).slice(0, maxSalons);
      ...
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="max-w-2xl">
          ...
          <p className="text-muted-foreground">
            Browse our complete collection of verified beauty and grooming establishments.
          </p>
        </div>
