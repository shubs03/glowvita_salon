  ...
  return (
    <section className="py-8 sm:py-10 lg:py-12 overflow-hidden bg-white">
      ...
      <div className="relative">
        ...
        {/* Content Area */}
        {isLoading ? (
          <div className="flex gap-6 sm:gap-8 lg:gap-10 px-4 sm:px-6 justify-center">
          ...
          <div className="flex overflow-hidden w-full">
            <div className={`flex ${isLoopingRequired ? 'animate-marquee hover:[animation-play-state:paused] w-max' : 'w-full'} whitespace-nowrap py-4 px-4 sm:px-6`}>
              {marqueeItems.map((offer, index) => (
                <Link
                  key={index}
                  href={`/book/${offer.vendorId}?offerCode=${offer.code}`}
                  className="relative flex-shrink-0 inline-block px-3 sm:px-4 lg:px-5"
