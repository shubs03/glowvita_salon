...
const ServiceCard = ({ icon, title, description, className }: { icon: React.ReactNode, title: string, description: string, className?: string }) => (
    <div className={cn("group relative rounded-2xl bg-gradient-to-br from-background via-background to-secondary/20 p-6 shadow-lg border border-border/50 hover:border-primary/50 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-primary/10 overflow-hidden", className)}>
        ...
        <div className="relative z-10">
            <div className="bg-primary/10 text-primary w-14 h-14 rounded-2xl flex items-center justify-center mb-5 shadow-md border border-primary/20 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300">
                {icon}
            </div>
            <h3 className="text-lg font-bold mb-2 text-foreground">{title}</h3>
            <p className="text-muted-foreground leading-relaxed mb-5 text-sm">{description}</p>
            <a href="#" className="font-semibold text-primary inline-flex items-center group/link text-sm">
                Learn More
