...
export function SalonsSection() {
  ...
  // Fetch vendor data using RTK Query
  const { data: vendorsData, isLoading, isError, error } = useGetVendorsQuery(undefined);

  // Log the data to the console when it's available
  useEffect(() => {
    console.log("--- Vendor Data from SalonsSection ---");
    ...
    console.log("------------------------------------");
  }, [vendorsData, isLoading, isError, error]);
