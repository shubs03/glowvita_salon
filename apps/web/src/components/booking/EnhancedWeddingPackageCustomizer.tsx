...
import { Minus, Plus, Check, X, Heart, Scissors, Clock, IndianRupee } from 'lucide-react';
import { cn } from '@repo/ui/cn';
import { Service } from '@/hooks/useBookingData';
import { EnhancedWeddingPackage, EnhancedWeddingPackageService } from '@/hooks/useEnhancedWeddingPackages';
...
interface EnhancedWeddingPackageCustomizerProps {
    weddingPackage: EnhancedWeddingPackage;
    allServices: Service[];
    onPackageUpdate: (updatedPackage: EnhancedWeddingPackage, selectedServices: Service[]) => void;
    onBack: () => void;
    currentStep: number;
    setCurrentStep: (step: number) => void;
}

export function EnhancedWeddingPackageCustomizer({ 
    weddingPackage, 
    allServices, 
    onPackageUpdate,
    onBack,
    currentStep,
    setCurrentStep
}: EnhancedWeddingPackageCustomizerProps) {
    ...
    const handleConfirmCustomization = () => {
        ...
        const updatedPackage: EnhancedWeddingPackage = {
            ...weddingPackage,
            services: selectedServices.map(service => ({
                ...
                isCustomized: true,
                customizations: {}
            })) as EnhancedWeddingPackageService[],
    ...
    return (
        <div className="w-full">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-4 mt-3">
                            <div className="bg-rose-100 text-rose-800 px-3 py-1 rounded-full text-sm font-medium flex items-center">
                        ...
                        {/* Deposit Information */}
                        {weddingPackage.depositRequired && (
                            <div className="mt-3 text-sm">
                                <span className="font-medium">Deposit Required:</span> 
                                {weddingPackage.depositAmount > 0 ? (
                                    <span> ₹{weddingPackage.depositAmount.toFixed(2)}</span>
                                ) : weddingPackage.depositPercentage > 0 ? (
                                    <span> {weddingPackage.depositPercentage}% (₹{(totalPrice * weddingPackage.depositPercentage / 100).toFixed(2)})</span>
                                ) : (
            ...
            {/* Cancellation Policy */}
            {weddingPackage.cancellationPolicy && (
                <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <h4 className="font-semibold text-yellow-800 mb-2">Cancellation Policy</h4>
                    <p className="text-sm text-yellow-700">{weddingPackage.cancellationPolicy}</p>
                </div>
            )}
